...
- `category_id`: nullable。指定 category と descendants を対象にする。
...
- `POST /api/v1/secret-unlocks` と `X-Secret-Unlock` 検証は実装済み。初期 baseline では user の account password hash を unlock password として検証し、15 分有効な user scoped token を返す。

### `POST /api/v1/secret-unlocks`

認証必須。user scoped unlock password を検証し、短時間有効な unlock token を返す。初期 baseline では専用 unlock password 設定 UI が未実装のため、user の account password hash を検証する。
...
- Laravel auth session には依存せず、API Base URL / Bearer token / unlock token を frontend state で扱う。
...
- frontend は `GET /api/v1/memory-space` に Bearer token を付け、period / category / descendant filter を query に反映する。
...
次の周辺 task は、管理画面モックアップの Categories で `parent_id` の最小入力 / 表示を実 API に接続し、category hierarchy の検証導線を揃えること。
